...
const Updates = () => {
  ...
  const handleApplyUpdate = async () => {
    ...
    const confirmed = window.confirm(
      'Lancer la mise à jour depuis le conteneur LXC ?\n\n' +
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div>
              <div className="mb-2 flex items-center gap-3">
                ...
                <h3 className="text-lg font-semibold text-gray-900">Mode d’action</h3>
              </div>
              <p className="text-lg font-semibold text-gray-900">{workflow?.mode || 'lxc_in_app'}</p>
              <div className="mt-2">
                <StatusBadge type="ok">Interface graphique • LXC</StatusBadge>
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <Card className={precheckResult.success ? 'border-green-200' : 'border-red-200'}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              {precheckResult.success ? <CheckCircle size={20} className="text-green-600" /> : <AlertCircle size={20} className="text-red-600" />}
              Résultat de la pré-vérification
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-gray-700">
              {precheckResult.success ? 'Les prérequis principaux sont validés.' : 'Un ou plusieurs prérequis sont en erreur. Consultez les logs.'}
            </p>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="bg-gray-900 text-white">
          ...
          <CardContent>
            <div className="max-h-64 overflow-y-auto space-y-1 font-mono text-sm">
              {updateLogs.map((line, index) => <div key={index}>{line}</div>)}
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <CardHeader className="cursor-pointer hover:bg-amber-50" onClick={() => { const next = !expandedServerLog; setExpandedServerLog(next); if (next && !serverLog) loadServerLog(false); }}>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-amber-700">
              ...
              {serverLogInfo?.in_progress && <StatusBadge type="warning">En cours</StatusBadge>}
            </CardTitle>
            <div className="flex items-center gap-2">
              {expandedServerLog && (
                <Button variant="ghost" size="sm" className="text-amber-700" onClick={(e) => { e.stopPropagation(); loadServerLog(true); }} disabled={serverLogLoading}>
      ...
      <Card>
        <CardHeader className="cursor-pointer hover:bg-gray-50" onClick={() => setExpandedBackups(!expandedBackups)}>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2"><RotateCcw size={20} />Sauvegardes locales <span className="text-sm font-normal text-gray-500">({backups.length})</span></CardTitle>
                  ...
                  <div key={backup.path} className="rounded-lg border bg-gray-50 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{backup.type === 'mongodb' ? 'Sauvegarde MongoDB' : 'Sauvegarde applicative'}</p>
                      </div>
      ...
      {changelog.length > 0 && (
        <Card>
          <CardHeader className="cursor-pointer hover:bg-gray-50" onClick={() => setExpandedChangelog(!expandedChangelog)}>
            <div className="flex items-center justify-between"><CardTitle>📝 Nouveautés</CardTitle>{expandedChangelog ? <ChevronDown size={20} /> : <ChevronRight size={20} />}</div>
          </CardHeader>
          {expandedChangelog && (
            <CardContent>
              <div className="space-y-4">
                {changelog.map((log, index) => (
                  <div key={`${log.version || index}-${index}`}>
                    ...
                    <ul className="space-y-1 text-sm text-gray-700">{(log.changes || []).map((change, idx) => <li key={idx}>• {change}</li>)}</ul>
                  </div>
                ))}
              </div>
            </CardContent>
      ...
      <Card>
        <CardHeader className="cursor-pointer hover:bg-gray-50" onClick={() => setExpandedHistory(!expandedHistory)}>
          <div className="flex items-center justify-between"><CardTitle className="flex items-center gap-2"><Clock size={20} />Historique des mises à jour</CardTitle>{expandedHistory ? <ChevronDown size={20} /> : <ChevronRight size={20} />}</div>
        </CardHeader>
        {expandedHistory && (
          <CardContent>
            {history.length === 0 ? <p className="py-4 text-center text-gray-500">Aucune mise à jour enregistrée.</p> : (
                  ...
                  <div key={item.id || index} className="rounded-lg bg-gray-50 p-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {item.duration_seconds && <p className="text-xs text-gray-500">Durée : {Math.floor(item.duration_seconds / 60)}m {Math.floor(item.duration_seconds % 60)}s</p>}
                        {item.summary?.errors?.length > 0 && <div className="mt-2 text-xs text-red-600">{item.summary.errors.map((err, i) => <p key={i}>❌ {err}</p>)}</div>}
                      </div>
                    </div>
                  </div>
                ))}
